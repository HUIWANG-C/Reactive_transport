...
##--------------------------- Case set --------------------------------------##
source parms
echo -e "Simulation case: set$setid--unsaturated\n" > zlog_cmd
...
lx="`echo "$x1 - $x0" | bc -l`"
ly="`echo "$y1 - $y0" | bc -l`"
x_cells="`echo "($lx + 2 * $buff) * $discr" | bc -l`"
x_cells="`printf "%.0f\n" $(bc <<< $x_cells)`"
y_cells="`echo "$ly * $discr" | bc -l`"
y_cells="`printf "%.0f\n" $(bc <<< $y_cells)`"
x0_full="`echo "$x0 - $buff" | bc -l`"
x1_full="`echo "$x1 + $buff" | bc -l`"
buffvol=$(awk -v buff="$buff" -v ly="$ly" -v scxy="$scale_xy" -v scz="$scale_z" 'BEGIN{printf("%.15G",2*buff*ly*scxy*scxy*scz)}')
...
echo -e "Finish running simpleHeleShaw\n" >> zlog_cmd
